...
public class BoardController {
    ...
    public String detail(@PathVariable("boardId") Long boardId, Model model,
        ...
        if (loginUser != null) {
            // 사용자와 아이디가 같으면 isWriter 참
            boolean isWriter = board.getWriter().equals(loginUser.getLoginId());
            // 사용자의 권한이 ROLE_ADMIN이면 isAdmin 참
    ...
    public String showCreateForm(Model model, @AuthenticationPrincipal CustomUserDetails loginUser) {
    ...
    public String create(@Valid BoardForm boardForm, BindingResult bindingResult, 
                         @AuthenticationPrincipal CustomUserDetails loginUser) throws Exception {
    ...
    public String showEditForm(@PathVariable("boardId") Long boardId, Model model, @AuthenticationPrincipal CustomUserDetails loginUser) {
        Board board = this.boardService.readBoardById(boardId);
