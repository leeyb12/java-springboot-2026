...
- 사용가능한 곳 
    ...
    - 순수 안드로이드 개발
    - 이외 모든 분야 사용가능
    - 시스템 프로그래밍, 높은 성능 요구, iOS 모바일 개발은 어려움
...
- 중요 용어
    - JDK : Java Development Kit. 자바 개발하려면 필수로 설치해야하는 키트
    - JRE : Java Runtime Environment. JDK보다 작은 그룹. 자바 실행할 수 있는 파일만 존재
    - JVM : Java Virtual Machine. 자바로 컴파일된 바이트코드를 실행시키는 가상머신. JDE/JDK 포함
      - OS: 플랫폼 독립적으로 동작. 윈도우에 개발한 자바도 리눅스 등에서 실행 가ㄴ,ㅇ
...
- 프로젝트 빌드, 자바 버전, 자바 의존성(라이브러리) 관리, 문서화, 프로젝트 전체 관리
    - Maven : 오래된 빌드도구. xml 기반. pom.xml 로 관리
...
- settings.gradle : 프로젝트 명, 멀티 프로젝트 구성 정의. 구성요소 정의
...
```
- TIP : Gradle for Java로 프로젝트 생성 후 src/test/java/.../AppText.java 지우고 진행
