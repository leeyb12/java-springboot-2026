...
├─ dto : 패턴 중 Model에 직접연관(DB 테이블 매핑)
...
- powershell에서 구글클라이언트아이디와 비밀키를 윈도우 환경변수에 등록
  ...
  성공: 지정한 값을 저장했습니다.
  > setx GOOGLE_CLIENT_SECRET "본인_구글클라리언트_시크릿키"
...
- 소셜 로그인용 연결용 테이블 생성
- 기존 로그인 테이블 password 필드 NOT NULL -> NULL로 변경(소셜로그인으로는 패스워드 전달안됨)
